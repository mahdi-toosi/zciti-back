...
type TestApp struct {
	...
	Cleanup      func()
	CouponRepo   repository.IRepository
...
func getProjectRoot() string {
	...
	// Navigate from app/module/coupon/test/ to project root
...
// migrateTestModels creates the necessary tables for coupon testing
func migrateTestModels(db *gorm.DB) error {
	...
	db.Exec("CREATE INDEX IF NOT EXISTS idx_coupons_business_id ON coupons(business_id)")
	db.Exec("CREATE INDEX IF NOT EXISTS idx_coupons_deleted_at ON coupons(deleted_at)")
...
func SetupTestApp(t *testing.T) *TestApp {
	...
	// Create repositories
	couponRepo := repository.Repository(dbWrapper)
	...
	couponSvc := service.Service(couponRepo, userSvc, mockMW)

	// Create coupon controller
	couponController := controller.Controllers(couponSvc)

	// Create coupon router manually (since newRouter is unexported)
	couponRouter := &coupon.Router{
		App:        app,
		Controller: couponController,
	}
	couponRouter.RegisterRoutes(cfg)
...
// CleanupCoupons removes all coupons from the test database
func (ta *TestApp) CleanupCoupons(t *testing.T) {
	t.Helper()
