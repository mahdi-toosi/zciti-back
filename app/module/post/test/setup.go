...
type TestApp struct {
	...
	Cleanup    func()
	PostRepo   repository.IRepository
...
func getProjectRoot() string {
	...
	// Navigate from app/module/post/test/ to project root
...
// migrateTestModels creates the necessary tables for post testing
...
func SetupTestApp(t *testing.T) *TestApp {
	...
	// Create repositories
	postRepo := repository.Repository(dbWrapper)

	// Create post service
	postSvc := service.Service(postRepo)

	// Create post controller
	postController := controller.Controllers(postSvc)

	// Create post router manually (since newRouter is unexported)
	postRouter := &post.Router{
		App:        app,
		Controller: postController,
	}
	postRouter.RegisterRoutes(cfg)
...
func (ta *TestApp) CreateTestPost(t *testing.T, title, content, excerpt string, status schema.PostStatus, postType schema.PostType, authorID, businessID uint64) *schema.Post {
	t.Helper()
